...
import {
  getRefineryById,
  REFINERY_TYPE_LABELS,
  type RefineryProperties,
} from '@/lib/refineries';
...
export default function RefineryCard({ selectedId, onClose }: Props) {
  const [displayed, setDisplayed] = useState<RefineryProperties | null>(null);
  const isOpen = selectedId !== null;

  useEffect(() => {
    if (selectedId) {
      const r = getRefineryById(selectedId);
      if (r) setDisplayed(r);
    } else {
      ...
      {displayed && (
        <>
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs uppercase tracking-widest text-neutral-500">
                  {REFINERY_TYPE_LABELS[displayed.type]} · {displayed.location.city},{' '}
                  {displayed.location.province}
                </p>
              </div>
          ...
          <div className="flex-1 space-y-5 overflow-y-auto px-6 py-5">
            ...
            <Field label="Inputs (what it refines)">{displayed.inputs}</Field>
            <Field label="Outputs (products)">{displayed.outputs}</Field>
            <Field label="Process (how it works)">{displayed.process}</Field>
