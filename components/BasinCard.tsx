...
import { getBasinById, RESOURCE_TYPE_LABELS, type BasinProperties } from '@/lib/basins';
...
export default function BasinCard({ selectedId, onClose }: Props) {
  const [displayed, setDisplayed] = useState<BasinProperties | null>(null);
  const isOpen = selectedId !== null;

  useEffect(() => {
    if (selectedId) {
      const b = getBasinById(selectedId);
      if (b) setDisplayed(b);
    } else {
      ...
      {displayed && (
        <>
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs uppercase tracking-widest text-neutral-500">
                  {RESOURCE_TYPE_LABELS[displayed.resourceType]} · {displayed.province}
                </p>
              </div>
              <h2 className="mt-1.5 font-serif text-2xl text-neutral-900 dark:text-neutral-50">
                {displayed.name}
              </h2>
              <p className="mt-1 text-sm text-neutral-500">{displayed.primaryResource}</p>
            </div>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="flex-1 space-y-5 overflow-y-auto px-6 py-5">
            <Field label="Production">{displayed.productionContext}</Field>
            <Field label="Reserves / resource">{displayed.reserves}</Field>
            <Field label="Major operators">{displayed.majorOperators.join(' · ')}</Field>
            <Field label="Geology">{displayed.geology}</Field>
